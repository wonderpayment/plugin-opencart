...
class PaymentSDK
{
    ...
    public function __construct($options) {
        ...
        // 调试日志 - 尝试写入文件
        $debugLog = date('Y-m-d H:i:s') . ' - PaymentSDK Constructor: environment = ' . $environment . ', jwtToken = ' . (empty($this->jwtToken) ? 'EMPTY' : 'SET') . ', userAccessToken = ' . (empty($this->userAccessToken) ? 'EMPTY' : 'SET') . PHP_EOL;
        file_put_contents(DIR_STORAGE . 'logs/wonderpayment_debug.log', $debugLog, FILE_APPEND);
    }
    ...
    public function createQRCodeShortLink($uuid)
    {
        ...
        // 调试日志
        error_log('createQRCodeShortLink: jwtToken = ' . (empty($this->jwtToken) ? 'EMPTY' : 'SET'));
    ...
    {
        // 调试日志
        $debugLog = date('Y-m-d H:i:s') . ' - getBusinesses: userAccessToken = ' . (empty($this->userAccessToken) ? 'EMPTY' : 'SET (' . strlen($this->userAccessToken) . ' chars)') . PHP_EOL;
        file_put_contents(DIR_STORAGE . 'logs/wonderpayment_debug.log', $debugLog, FILE_APPEND);
